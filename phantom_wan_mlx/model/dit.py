...
def forward(
    ...
    context: mx.array,             # [seq, 4096] or [1, seq, 1536]
    rope=None,
    seq_len: int = None,
    teacache=None,
    ...
    # 3. Project umT5 context [seq, 4096] -> [1, seq, 1536]
    ctx_in = context
    if ctx_in.ndim == 2:
        ctx_in = ctx_in[None]
    ...
    # 5. Call WanModel with required seq_len positional argument
    out = model(x_in, t=t, context=ctx_in, seq_len=seq_len)

    # 6. Squeeze batch dimension [1, 16, F, H, W] -> [16, F, H, W]
