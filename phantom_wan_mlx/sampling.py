...
def sample_s2v(
    ...
    cfg,                          # PhantomWanConfig / WanModelConfig
    ...
    # Compute RoPE frequencies and grid size for total 23 frames (21 target + 2 refs)
    ...
    for i, t in enumerate(sched.timesteps):
        ...
        mx.eval(latent)                                                 # Metal evaluation boundary
        
        if verbose:
            print(f"  step {i + 1}/{steps}", flush=True)

    return latent[:, :-k]                                               # Strip reference tail -> [16, 21, 32, 64]
